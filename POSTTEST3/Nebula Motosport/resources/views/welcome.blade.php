    ...
    <section id="products" class="container mx-auto px-6 py-20 max-w-7xl">
        ...
        <div class="grid gap-12 md:grid-cols-4">
            @foreach ($products as $product)
                <article
                    class="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-shadow duration-300 overflow-hidden group cursor-pointer">
                    {{-- Gambar --}}
                    <div class="overflow-hidden rounded-t-2xl flex justify-center bg-gray-50">
                    ...
                    {{-- Info Produk --}}
                    <div class="p-6">
                        <h3 class="text-xl font-semibold text-gray-900 mb-1 tracking-wide">{{ $product['name'] }}</h3>
                        <p class="text-sm text-gray-500 mb-3">Sport Bike</p>

                        {{-- Release Date --}}
                        <p class="text-sm text-gray-600 mb-4">
                            Release: {{ \Carbon\Carbon::parse($product['release_date'])->format('d M Y') }}
                        </p>

                        {{-- Harga --}}
                        <div class="flex items-baseline gap-2 mb-6">
                            <p class="text-indigo-600 font-bold text-lg">${{ number_format($product['price'], 0) }}</p>
                            <span class="text-gray-500 text-sm">/unit</span>
                        </div>

                        {{-- Tombol --}}
                        <a href="{{ route('product.details', $product->id) }}"
                            class="block text-center bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 rounded-full shadow-md">
                            Buy
                        </a>
                    </div>
                </article>
